            ...
            if(!$ajax_load_popups || $mode == 'mb') {
                if ($level == 0 && ($blockType == 'wide' || $blockType == 'staticwidth') ) {
                    ...
                    if (count($activeChildren)) {
                        ...
                        $html[] = '<div class="row">';                    
                        if ($blockType == 'staticwidth') {                   
                            $html[] = '<ul style="float:left;width: 35%;">';
                        } else {                   
                            $html[] = '<ul>';
                        }
                        $html[] = $this->drawColumns($activeChildren, $columns, count($activeChildren),'', 'wide');
                        $html[] = '</ul>';
                        $mCatModel = Mage::getModel('catalog/category')->load($category->getId());
                        if ($blockType == 'staticwidth' && $mCatModel->getThumbnail()) {
                            $html[] = '<div class="category_img" style="float:right;width: 65%;text-align: right;padding-right: 30px;">';
                            $html[]  .= '<div class="menu_thumb_img">';
                            $html[]  .= '<a class="menu_thumb_link" href="'. $this->getCategoryUrl($category) .'">';
                            $html[]  .= '<img src="' . Mage::getBaseUrl('media').'catalog/category/' . $mCatModel->getThumbnail() . '" alt="' . Mage::helper('catalog/output')->__("Thumbnail Image").'" />';
                            $html[]  .= '</a>';
                            $html[]  .= '</div>';
                            $html[]  .= '</div>';
        ...
        foreach ($children as $child)
        {
            if (is_object($child) && $child->getIsActive())
            {
                ...
                if ($level == 1) {
                    ...
                    //     $img = $catModel->getThumbnail();
                    // } else {
                    //     $img = '_text_3.png';
                    // }
                    // if (($type == 'wide' || $type == 'staticwidth') && $img) {
                    //     $html .= '<div class="menu_thumb_img" style="float:right;display:none">';
                    //     $html .= '<a class="menu_thumb_link" href="'. $this->getCategoryUrl($child) .'">';
                    //     $html .= '<img src="' . Mage::getBaseUrl('media').'catalog/category/' . $img . '" alt="' . Mage::helper('catalog/output')->__("Thumbnail Image").'" />';
                    //     $html .= '</a>';
                    //     $html .= '</div>';
                    // }
